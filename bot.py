...
class AutoFillForm:
        ...
        try:
            ...
            if apply_info.town == "板橋區":
                station_select = Select(
                    self.wait.until(
                        EC.element_to_be_clickable((By.ID, "policeStationsData"))
                    )
                )
                station_select.select_by_visible_text(
                    get_police_precinct(apply_info.village)
                )
            ...
            Select(
                self.wait.until(EC.presence_of_element_located((By.ID, "asdHour")))
            ).select_by_visible_text(f"{apply_info.start_datetime.hour:02d}")
            Select(
                self.wait.until(EC.presence_of_element_located((By.ID, "asdMinute")))
            ).select_by_visible_text(f"{apply_info.start_datetime.minute:02d}")

            end_datetime = apply_info.start_datetime + apply_info.duration
            self.wait.until(
                EC.presence_of_element_located((By.ID, "applyEndDt"))
            ).send_keys(end_datetime.strftime("%Y-%m-%d"))
            Select(
                self.wait.until(EC.presence_of_element_located((By.ID, "aedHour")))
            ).select_by_visible_text(f"{end_datetime.hour:02d}")
            Select(
                self.wait.until(EC.presence_of_element_located((By.ID, "aedMinute")))
            ).select_by_visible_text(f"{end_datetime.minute:02d}")

            self.wait.until(
                EC.presence_of_element_located((By.ID, "villageName"))
            ).send_keys(apply_info.chief_name)
            today = date.today()
            self.wait.until(
                EC.presence_of_element_located((By.ID, "villageNotifyDt"))
            ).send_keys((today - timedelta(days=1)).strftime("%Y-%m-%d"))
            self.wait.until(
                EC.presence_of_element_located((By.ID, "residendtName"))
            ).send_keys(input_value.resident)
            self.wait.until(
            ...
            self.driver.execute_script(
                "window.alert('自動填入完成，請確認資料正確後點選下一步！');"
            )
    ...
    def scroll_to_table(self):
        ...
        target_element = self.wait.until(
            EC.presence_of_element_located((By.XPATH, '//*[@id="acSnapShot"]/div[3]'))
        )
    ...
    def flashing_submit_button(self):
        ...
        next_btn = self.wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, "//button[@data-ng-click='doSend(ac125024);']")
            )
        )
        ...
        # 4. 加上 class
        self.driver.execute_script(
            "arguments[0].classList.add('selenium-flashing-force');", next_btn
        )
    ...
    def solve_and_fill_captcha(self, refresh: bool = False):
        ...
        if refresh:
            refresh_element = self.wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, "//a[contains(@href, 'reloadCaseQuery')]")
                )
            )
        ...
        code_text = pytesseract.image_to_string(
            img, config=r"--psm 8 -c tessedit_char_whitelist=0123456789"
        ).strip()
        logger.info("辨識到的驗證碼為: %s", code_text)

        auth_input = self.wait.until(
    ...
    def retry_on_captcha_error(self, max_retries: int = 20):
        for attempt in range(max_retries):
            if not self._captcha_error_popup_present():
                return True
            logger.warning(
                "偵測到驗證碼錯誤，重新驗證 (%s/%s)", attempt + 1, max_retries
            )
    ...
    def _captcha_error_popup_present(self, timeout: float = 1.0) -> bool:
        ...
        while time.time() < deadline:
            ...
            for el in self.driver.find_elements(
                By.XPATH, "//*[contains(normalize-space(.), '驗證碼錯誤')]"
            ):
                try:
                    if el.is_displayed():
    ...
    def upload_file(self, index: int, path: str):
        btn = self.driver.find_element(
            By.XPATH, f"(//input[@value='上傳檔案'])[{index + 1}]"
        )
        js = """
        ...
        file_inp = self.driver.find_element(
            By.XPATH, f"(//input[@type='file' and @id='testF1'])[{index + 1}]"
        )
    ...
    def parse_completion_and_log(
        self, log_file="application_success.log", timeout: int = 60 * 60 * 24
    ):
        ...
        try:
            ...
            _ = long_wait.until(
                EC.visibility_of_element_located((By.CLASS_NAME, "completed-massage"))
            )
